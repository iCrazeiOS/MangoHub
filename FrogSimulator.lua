...
getgenv().autoGemsEnabled = false


local MainUI = UILibrary.Load("Frog Simulator Menu - by iCraze")
...
local autoClickerToggle = autoPage.AddToggle("Auto Clicker", false, function(Value)
	...
	spawn(function()
		while wait(0.25) do
			if getgenv().autoClickEnabled then game:GetService("ReplicatedStorage").NetActivation:FireServer()
			else break end
		end
	end)
end)

local autoGemsToggle = autoPage.AddToggle("Auto Grab Gems", false, function(Value)
	getgenv().autoGemsEnabled = Value
	spawn(function()
		while wait(2) do
			if getgenv().autoGemsEnabled then
